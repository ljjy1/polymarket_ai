...
# get config address from be.yml
configHost=$(bash scripts/parseYaml.sh configs/be.yml '.app.host')
if [ "${configHost}" = "127.0.0.1" ]; then
  configHost="localhost"
fi
configPort=$(bash scripts/parseYaml.sh configs/be.yml '.http.port')
configAddr="${configHost}:${configPort}"
enableMode=$(bash scripts/parseYaml.sh configs/be.yml '.http.tls.enableMode')
schemes="http"
if [ "$enableMode" != "" ]; then
  schemes="https"
fi
...
swaggerAddr=$(grep -E '^[[:space:]]*//[[:space:]]*@host' "cmd/be/main.go" | awk '{print $3}')
if [[ -z "${swaggerAddr}" ]]; then
  swaggerAddr="localhost:8080"
fi
if [ "${configAddr}" != "${swaggerAddr}" ];then
  sed -i "s/${swaggerAddr}/${configAddr}/g" cmd/be/main.go
fi

# generate api docs
swag init -g cmd/be/main.go
checkResult $?
