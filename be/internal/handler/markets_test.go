...
func newMarketsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewMarketsDao(d.DB, c.ICache.(cache.MarketsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &marketsHandler{iDao: d.IDao.(dao.MarketsDao)}
	iHandler := h.IHandler.(MarketsHandler)
...
func Test_marketsHandler_Create(t *testing.T) {
	h := newMarketsHandler()
	defer h.Close()
	testData := &types.CreateMarketsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Markets))
...
func Test_marketsHandler_DeleteByID(t *testing.T) {
	h := newMarketsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Markets)
...
func Test_marketsHandler_UpdateByID(t *testing.T) {
	h := newMarketsHandler()
	defer h.Close()
	testData := &types.UpdateMarketsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Markets))
...
func Test_marketsHandler_GetByID(t *testing.T) {
	h := newMarketsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Markets)
...
func Test_marketsHandler_List(t *testing.T) {
	h := newMarketsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Markets)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListMarketssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewMarketsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewMarketsHandler()
