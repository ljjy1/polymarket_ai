...
func newPredictionsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewPredictionsDao(d.DB, c.ICache.(cache.PredictionsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &predictionsHandler{iDao: d.IDao.(dao.PredictionsDao)}
	iHandler := h.IHandler.(PredictionsHandler)
...
func Test_predictionsHandler_Create(t *testing.T) {
	h := newPredictionsHandler()
	defer h.Close()
	testData := &types.CreatePredictionsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Predictions))
...
func Test_predictionsHandler_DeleteByID(t *testing.T) {
	h := newPredictionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Predictions)
...
func Test_predictionsHandler_UpdateByID(t *testing.T) {
	h := newPredictionsHandler()
	defer h.Close()
	testData := &types.UpdatePredictionsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Predictions))
...
func Test_predictionsHandler_GetByID(t *testing.T) {
	h := newPredictionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Predictions)
...
func Test_predictionsHandler_List(t *testing.T) {
	h := newPredictionsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Predictions)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListPredictionssRequest{query.Params{
		Page:  0,
		Limit: 10,
