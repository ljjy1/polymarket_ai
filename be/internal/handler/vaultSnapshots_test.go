...
func newVaultSnapshotsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewVaultSnapshotsDao(d.DB, c.ICache.(cache.VaultSnapshotsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &vaultSnapshotsHandler{iDao: d.IDao.(dao.VaultSnapshotsDao)}
	iHandler := h.IHandler.(VaultSnapshotsHandler)
...
func Test_vaultSnapshotsHandler_Create(t *testing.T) {
	h := newVaultSnapshotsHandler()
	defer h.Close()
	testData := &types.CreateVaultSnapshotsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.VaultSnapshots))
...
func Test_vaultSnapshotsHandler_DeleteByID(t *testing.T) {
	h := newVaultSnapshotsHandler()
	defer h.Close()
	testData := h.TestData.(*model.VaultSnapshots)
...
func Test_vaultSnapshotsHandler_UpdateByID(t *testing.T) {
	h := newVaultSnapshotsHandler()
	defer h.Close()
	testData := &types.UpdateVaultSnapshotsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.VaultSnapshots))
...
func Test_vaultSnapshotsHandler_GetByID(t *testing.T) {
	h := newVaultSnapshotsHandler()
	defer h.Close()
	testData := h.TestData.(*model.VaultSnapshots)
...
func Test_vaultSnapshotsHandler_List(t *testing.T) {
	h := newVaultSnapshotsHandler()
	defer h.Close()
	testData := h.TestData.(*model.VaultSnapshots)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListVaultSnapshotssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewVaultSnapshotsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewVaultSnapshotsHandler()
}
