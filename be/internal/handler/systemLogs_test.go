...
func newSystemLogsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewSystemLogsDao(d.DB, c.ICache.(cache.SystemLogsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &systemLogsHandler{iDao: d.IDao.(dao.SystemLogsDao)}
	iHandler := h.IHandler.(SystemLogsHandler)
...
func Test_systemLogsHandler_Create(t *testing.T) {
	h := newSystemLogsHandler()
	defer h.Close()
	testData := &types.CreateSystemLogsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.SystemLogs))
...
func Test_systemLogsHandler_DeleteByID(t *testing.T) {
	h := newSystemLogsHandler()
	defer h.Close()
	testData := h.TestData.(*model.SystemLogs)
...
func Test_systemLogsHandler_UpdateByID(t *testing.T) {
	h := newSystemLogsHandler()
	defer h.Close()
	testData := &types.UpdateSystemLogsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.SystemLogs))
...
func Test_systemLogsHandler_GetByID(t *testing.T) {
	h := newSystemLogsHandler()
	defer h.Close()
	testData := h.TestData.(*model.SystemLogs)
...
func Test_systemLogsHandler_List(t *testing.T) {
	h := newSystemLogsHandler()
	defer h.Close()
	testData := h.TestData.(*model.SystemLogs)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListSystemLogssRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewSystemLogsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewSystemLogsHandler()
}
