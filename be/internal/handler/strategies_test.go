...
func newStrategiesHandler() *gotest.Handler {
	...
	d.IDao = dao.NewStrategiesDao(d.DB, c.ICache.(cache.StrategiesCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &strategiesHandler{iDao: d.IDao.(dao.StrategiesDao)}
	iHandler := h.IHandler.(StrategiesHandler)
...
func Test_strategiesHandler_Create(t *testing.T) {
	h := newStrategiesHandler()
	defer h.Close()
	testData := &types.CreateStrategiesRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Strategies))
...
func Test_strategiesHandler_DeleteByID(t *testing.T) {
	h := newStrategiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Strategies)
...
func Test_strategiesHandler_UpdateByID(t *testing.T) {
	h := newStrategiesHandler()
	defer h.Close()
	testData := &types.UpdateStrategiesByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Strategies))
...
func Test_strategiesHandler_GetByID(t *testing.T) {
	h := newStrategiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Strategies)
...
func Test_strategiesHandler_List(t *testing.T) {
	h := newStrategiesHandler()
	defer h.Close()
	testData := h.TestData.(*model.Strategies)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListStrategiessRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewStrategiesHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewStrategiesHandler()
}
