...
const staticTestDir = "static/test"

var (
	apiV1RouterFns       []func(r *gin.RouterGroup) // group router functions (need jwt auth)
	apiV1PublicRouterFns []func(r *gin.RouterGroup) // group router functions (public, no jwt auth)
...
func NewRouter() *gin.Engine {
	...
	if config.Get().App.Env != "prod" {
		...
		// register metamask auth test page
		r.StaticFile("/test", staticTestDir+"/index.html")
	}

	// initialize jwt auth
	jwtCfg := config.Get().JWT
	auth.InitAuth([]byte(jwtCfg.SignKey), time.Hour*time.Duration(jwtCfg.ExpireTime),
		auth.WithInitAuthIssuer(jwtCfg.Issuer),
	)

	// register public routers (no jwt auth required)
	registerRouters(r, "/api/v1", apiV1PublicRouterFns)

	// register protected routers (jwt auth required)
	registerRouters(r, "/api/v1", apiV1RouterFns, auth.Auth())
